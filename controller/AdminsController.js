...
exports.updateRoles = (req, res, next) => {
  ...
  if (req.auth.userId === id && !role.includes("admin")) {
    return res.status(400).json({
      message: "Vous ne pouvez pas vous retirer vous-même votre rôle admin",
    });
  }

  Users.findByIdAndUpdate(id, role, { new: true, runValidators: true })
    .then((rolesUpdated) => {
      if (!rolesUpdated)
    ...
    .catch((error) =>
      resizeTo
        .status(400)
        .json({ message: "Erreur lors de la mise à jour des rôles" }),
    );
};
